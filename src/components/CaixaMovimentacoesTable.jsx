              ...
              return (
                <tr key={mov.id} className="hover:bg-white/5 transition-colors odd:bg-[#151b26] even:bg-[#1a2332]">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 font-bold uppercase text-xs px-2.5 py-1 rounded-full bg-opacity-10 ${typeConfig.color.replace('text-', 'bg-')} ${typeConfig.color}`}>
                      {typeConfig.icon}
                      {mov.tipo}
                    </span>
                  </td>
